...
class heavyexample3
{
    public:
        static void example(vector<unique_ptr<API>> &apis, SymbolTable &root)
        {
            ...
            // Pre-Condition
            //  call : deleteStudent(t : Token, studentId : string) ==> (OK)
            ...
            auto call = std::make_unique<APIcall>(make_unique<FuncCall>("deleteStudent", move(call_args1)), Response(HTTPResponseCode::OK_200, std::make_unique<Var>("studentData"))); // to be fixed as student is of type var here
            // POSTCONDITION: S' = S \ {studentId}
            vector<unique_ptr<Expr>> post_args;
            vector<unique_ptr<Expr>> dashParams,notInParams1,minusParams1;
            unique_ptr<Expr> minus1;
            dashParams.push_back(std::make_unique<Var>("S"));
            equalsParams2.push_back(std::make_unique<FuncCall>("\'", move(dashParams)));
            minusParams1.push_back(std::make_unique<Var>("S"));
            minusParams1.push_back(std::make_unique<Var>("studentId"));
            minus1 = std::make_unique<FuncCall>("\\",move(minusParams1));
            equalsParams2.push_back(move(minus1));
            ...
            SymbolTable *sym6 = new SymbolTable();
            sym6->symtable.insert(Var("token"));
            sym6->symtable.insert(Var("studentId"));
            root.children.push_back(sym6);
        }
};
